...
print("Give Values for : id , campaign_id , coupon_id , customer_id")
id,campaign_id,coupon_id,customer_id = input().split()
lst = [int(id),int(campaign_id),int(coupon_id),int(customer_id)]
a_series = pd.Series(lst, index = test_data.columns)
test_data = pd.DataFrame()
test_data = test_data.append(a_series, ignore_index=True)
...
#Step 5 :- Load previously stored model


trained_model = pickle.load(open(filename, 'rb'))

#Step 6 :- Drop unwanted columns and prepare data for model
Y = test_data
...
#Step 7 :- Predict coupon redemption and print the results

Predictions = trained_model.predict(Y)
if(Predictions[0] == 0 ):
    print("Coupon won't be redeemed")
else:
    print("Coupon will be redeemed")


#  Sample Input :- 77211 2 1023 633 